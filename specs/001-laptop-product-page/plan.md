# Implementation Plan: Dell XPS 15 Product Detail Page
...
| SC-003 | Lighthouse desktop ≥ 90/95/95/95 (Perf/A11y/BP/SEO) | `@lhci/cli autorun` against a production build of `/products/xps-15`; thresholds declared in `.lighthouserc.json`; CI fails on regression | `.github/workflows/ci.yml` (`lighthouse` job) | T094 |
| SC-004 | Zero axe-core Serious/Critical findings on `/products/xps-15` | `@axe-core/playwright` scan on the production build; assertion fails if any violation has `impact === "serious"` or `"critical"` | `tests/e2e/accessibility.spec.ts` | T093 |
...
app/
├── layout.tsx
├── page.tsx                       ← redirects to /products/xps-15
└── products/
    └── [productId]/
